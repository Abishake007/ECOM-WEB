...
public class ProductController {
    ...
    public ResponseEntity<byte[]> getImageByProductId(@PathVariable("productId") Integer productId) {
        ...
        // Use the exact getter name your entity has
        ...
        try {
            mediaType = (imageType != null && !imageType.isBlank()) ?
                    MediaType.parseMediaType(imageType) : MediaType.IMAGE_JPEG;
        } catch (Exception ex) {
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=\"" +
                        (product.getImageName() == null ? ("product-" + productId) : product.getImageName()) + "\"")
    ...
    public ResponseEntity<List<Product>> searchProducts(@RequestParam String keyword){
        System.out.println("searchiing with" + keyword);
